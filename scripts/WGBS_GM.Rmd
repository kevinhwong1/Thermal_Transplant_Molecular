...
meth_table_filt <- meth_table[meth_table$sample_gene %in% newdata$vars,]
```
...
All_mat <- as.matrix(All_data[,-1])

pdf("../output/WGBS/Alldata.DMG_Rel_HEATMAP.pdf")
heatmap.2(All_mat, 
          cexCol = 1.5, 
...
PCA_all <- ggplot(pca_data_all, aes(PCA1, PCA2, color=OT, group = Col_Life)) + 
  ...
  geom_line() +
  scale_colour_manual(values=c("#ffae80", "#ff5d00", "#80d1ff", "#00a2ff")) +
